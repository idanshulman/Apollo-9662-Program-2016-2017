...
@Autonomous(name="REDYNEW", group="AUTONOMUS")
@Disabled
public class Autonomous_READY_NEW extends LinearOpMode {

    /* Declare OpMode members. */
    Hardware robot = new Hardware();   // Use a Pushbot's hardware
    private ElapsedTime runtime = new ElapsedTime();
    StopWatch_New stopwatch = new StopWatch_New();
    ...
    static final boolean TEAM_COLOR = true; // true = Red, false = Blue
    int COLOR;
    int OP_COLOR;
    @Override
    public void runOpMode(){
        ...
        turn(-42, 0.3);            // turn towards the beacon
        drive(100, 1);           // drives to get close to the line
        driveToLine(0.1);          // drives until the robot see the line
        drive(3, 0.3);             // drives a little bit further
        turn(-47, 0.3);// turn so the front of the robot is facing the beacon // was 51
        driveToDis(14, 0.12);
        beaconClick();
        fixbeacon();
        backToDistance(40, 0.3);   // get to distance from wall to throw balls
        throwBall(0.9);               // throw the balls
        //driveToDistance(13, 0.1);  // drives to the wall and click beacon
        driveToDis(30, 0.12);


                   // get away from beacon
        turn(105, 0.3);            // turn to the next beacon
        drive(64, 1);            // drives to get close to the line
        //driveToLine(0.1);          // drives to the line
        driveSecondLine(76, 0.1);
        drive(10, 0.4);             // drives a little bit further
        turn(-105, 0.3);           // turn to the beacon
        driveToDis(14, 0.12);  // drives to the wall and click beacon
        fixbeacon();
        drive(-13,0.5);
        turn(42, 0.3);
        drive(-150, 1);
    ...
    public void driveToDis(int distance, double power) {
        boolean hasPushed = false;
        //get a reading.
        double disR = robot.ultiR.getUltrasonicLevel();
        double disL = robot.ultiL.getUltrasonicLevel();
        setEncoderMode(false);
        setDirection(true);

        while(opModeIsActive() && (disR > distance|| disL > distance)){
            disR = robot.ultiR.getUltrasonicLevel();
            disL = robot.ultiL.getUltrasonicLevel();
            robot.Collector.setPower(1); //start the collector motor
    ...
    public boolean seeLine() {
        ...
        if (robot.ColorabiLReader.read8(8) > 15) {
            RetVal = true;
        }
        return RetVal;
    }
    ...
    public void fixbeacon (){


        sleep(2000);

        telemetry.addData("2 seconds sleep", "");
        if(robot.ColoradoReader.read8(4) == 3) {
